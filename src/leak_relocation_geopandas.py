...
def _spatial_reference_from_layer_json (layer_json ):
    ...
    raise RuntimeError (f'Layer metadata did not include a usable spatialReference: {sr }')
...
def get_layer_native_crs (session ,layer_key ):
    ...
    print (f'{layer_key }: native CRS assigned from MapServer [{layer_url }]: {crs .name }',flush =True )
    return crs 
...
def assign_native_crs (session ,layer_key ,gdf ):
    ...
    if existing is None :
        print (f'{layer_key }: dataframe CRS was empty; assigning native CRS {crs .name }',flush =True )
    else :
        print (f'{layer_key }: overriding dataframe CRS [{existing }] with native CRS [{crs .name }]',flush =True )
...
def to_analysis_crs (session ,layer_key ,gdf ):
    ...
    if gdf .crs !=analysis :
        print (f'{layer_key }: projecting from [{gdf .crs }] to analysis CRS [{analysis .name }]',flush =True )
...
def resolved_field (columns ,candidates ,required =False ,label ="field"):
    ...
    if resolved is not None :
        ...
        return resolved 
    if required :
        fail (f'Could not resolve required {label }. Candidates={candidates }. Available={available }')
...
def try_extract_oob_code_from_browser_history_since (start_epoch_seconds ):
    ...
    for history_path in browser_history_paths ():
        temp_path =os .path .join (tempfile .gettempdir (),f'arcgis_oauth_history_{abs (hash (history_path ))}.sqlite')
...
def interactive_access_token (session ):
    ...
    if "error"in data :
        fail (f'ArcGIS OAuth token request failed: {json .dumps (data ["error"],indent =2 )}')
...
def request_json (session ,url ,params =None ):
    ...
    if "error"in data :
        ...
        if code in ("498","499"):
            ...
            if "error"in data :
                fail (f'ArcGIS REST error after refreshed access token from {url }: {json .dumps (data .get ("error",{}),indent =2 )}')
        else :
...
def request_json_post (session ,url ,params ):
    ...
    if response .status_code >=400 :
        ...
        fail (f'ArcGIS POST failed for {url }. HTTP {response .status_code }. Response snippet: {snippet }')
    ...
    if "error"in data :
        ...
        if code in ("498","499"):
            ...
            if response .status_code >=400 :
                snippet =response .text [:1000 ]if response .text else ""
                fail (f'ArcGIS POST failed after refreshed token for {url }. HTTP {response .status_code }. Response snippet: {snippet }')

            data =response .json ()

            if "error"in data :
                fail (f'ArcGIS REST POST error after refreshed access token from {url }: {json .dumps (data .get ("error",{}),indent =2 )}')
        else :
...
def modified_field_from_meta (meta ,layer_name ):
    ...
    if field_name :
        ...
        warn (f'{layer_name }: no modified field found. Delta cache unavailable; full refresh will be used.')
    return field_name 
                ...
                or 'operatingpressure'in _lr_lower_values 
                    ...
                    for _lr_required_field in ['ASSETGROUP','ASSETTYPE']:
                        if _lr_required_field not in _lr_local_value :
                            ...
                            print (f'Added required DNV pipe domain field to outFields: {_lr_required_field }',flush =True )
...
def epoch_ms_to_sql_timestamp (epoch_ms ):
    ...
    try :
        ...
        warn (f'Delta watermark was not numeric [{epoch_ms }]. Falling back to full safe delta window.')
        return safe_default 
    try :
        if not math .isfinite (value ):
            warn (f'Delta watermark was not finite [{epoch_ms }]. Falling back to full safe delta window.')
            return safe_default 
    except Exception :
        pass 
    if value <=0 :
        warn (f'Delta watermark was <= 0 [{epoch_ms }]. Falling back to full safe delta window.')
        return safe_default 
    max_reasonable_epoch_ms =32503680000000.0 
    if value >max_reasonable_epoch_ms :
        warn (f'Delta watermark was outside valid millisecond range [{epoch_ms }]. Falling back to full safe delta window.')
        return safe_default 
    try :
        when =dt .datetime .fromtimestamp (value /1000.0 ,dt .UTC ).replace (tzinfo =None )
    except Exception as ex :
        warn (f'Could not convert delta watermark [{epoch_ms }] to timestamp: {ex }. Falling back to full safe delta window.')
        return safe_default 
...
def read_layer_cache (layer_name ,layer_url ,where_clause ):
    ...
    try :
        ...
        warn (f'{layer_name }: failed to read cache. Full refresh will be used. Error={ex }')
        return None ,None 
...
def query_feature_set (session ,layer_url ,where_clause ,layer_name ,meta ,out_fields ):
    ...
    if USE_OBJECTID_BATCH_DOWNLOAD :
        ...
        if not object_ids :
            log (f'{layer_name }: no OBJECTIDs returned for WHERE. Returning empty GeoDataFrame.')
        ...
        log (f'{layer_name }: downloading {len (object_ids ):,} OBJECTIDs in {total_batches :,} POST batches with {OBJECTID_DOWNLOAD_WORKERS } workers')
        ...
        with futures .ThreadPoolExecutor (max_workers =OBJECTID_DOWNLOAD_WORKERS )as executor :
            ...
            for future in futures .as_completed (future_lookup ):
                ...
                try :
                    ...
                    fail (f'{layer_name }: objectId POST batch {batch_number :,}/{total_batches :,} failed: {ex }')

                completed_batches +=1 

                if completed_batches ==1 or completed_batches %10 ==0 or completed_batches ==total_batches :
                    log (f'{layer_name }: completed {completed_batches :,}/{total_batches :,} batches; accumulated {len (features ):,} features')
...
def query_arcgis_layer (session ,layer_url ,where_clause ,layer_name ):
    ...
    if cached_gdf is not None and modified_field and cached_meta :
        ...
        if last_epoch_ms :
            ...
            if delta_count ==0 :
                if server_count is None or len (cached_gdf )==int (server_count ):
                    ...
                    return cached_gdf 
                warn (f'{layer_name }: no modified deltas, but count differs cached={len (cached_gdf ):,}, server={int (server_count ):,}. Full refresh required.')
            else :
                ...
                if server_count is not None and len (merged_gdf )!=int (server_count ):
                    warn (f'{layer_name }: merged cache count {len (merged_gdf ):,} does not equal server count {int (server_count ):,}. This usually means deletes occurred; doing full refresh.')
                else :
                    log (f'{layer_name }: delta refresh merged {len (delta_gdf ):,} changed records into cache.')
        ...
        log (f'{layer_name }: using local cache by count because modified field metadata is unavailable.')
        return cached_gdf 
...
def to_target_crs (gdf ,name ):
    ...
    if gdf .crs is None :
        warn (f'{name } has no CRS from service metadata. Assuming EPSG:3857 before projecting.')
...
def prepare_pipes (pipe_gdf ,layer_name ):
    ...
    if REQUIRE_PRESSURE_MATCH and not pressure_field :
        fail (f'REQUIRE_PRESSURE_MATCH is True but no pressure field was resolved on {layer_name }.')
    records =[]
    missing_material =0 
    missing_diameter =0 
...
def match_one_leak (task ):
    ...
    best =candidates [0 ]
    verbose (f'Leak {leak_number }: matched {best ["layer"]} pipe OID {best ["pipe_oid"]} at {best ["distance_ft"]:.2f} ft')
    return {
    "leak_oid":leak_oid ,
    "matched":True ,
    "layer":best ["layer"],
