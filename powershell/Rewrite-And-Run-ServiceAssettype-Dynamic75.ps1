...
$Log = Join-Path $Root ("LIVE_service_assettype_enrich_DYNAMIC75_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
if (!(Test-Path $Py)) { throw "Python venv not found: $Py" }
if (Test-Path $WorkPy) {
    Copy-Item $WorkPy ($WorkPy + "." + (Get-Date -Format "yyyyMMdd_HHmmss") + ".before_rewrite.bak") -Force
}
@'
...
if ($LASTEXITCODE -ne 0) { throw "Compile failed after complete rewrite: $WorkPy" }
$Log = Join-Path $Root ("LIVE_service_assettype_enrich_DYNAMIC75_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
Write-Host "Running service ASSETTYPE enrichment with dynamic 75pct workers..." -ForegroundColor Cyan
...
$Clip = @(
    "=== Service ASSETTYPE Dynamic75 Run ===",
    "ExitCode: $ExitCode",
    "Log: $Log",
    "",
    "--- LAST 220 LOG LINES ---",
...
$Clip | Set-Clipboard
if ($ExitCode -ne 0) { throw "Dynamic75 service ASSETTYPE enrichment failed. Tail copied to clipboard." }
