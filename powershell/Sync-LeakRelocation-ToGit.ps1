...
param(
    [string]$RepoUrl = "https://github.com/grantlella87-design/LeakRelocation.git",
    [string]$RepoDir = (Join-Path $env:USERPROFILE "Downloads\LeakRelocation"),
    [string]$Branch = "main",
...
# Native commands do not raise terminating errors, so $ErrorActionPreference = "Stop"
# does not catch a failed git call. Check the exit code explicitly instead.
# Pass arguments as an explicit array so tokens like -m and --prune are not
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "git pull --ff-only did not complete (exit code $LASTEXITCODE). Continuing with local branch state." -ForegroundColor DarkYellow
    }
...
Generated by `Sync-LeakRelocation-ToGit.ps1` on {{GENERATED}}.
...
'@
    $State = $StateTemplate.Replace("{{GENERATED}}", (Get-Date -Format "yyyy-MM-dd HH:mm:ss"))
