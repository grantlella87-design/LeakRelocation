...
Writes <out-dir>/<basename>.csv and .geojson. With --viewer, builds a Leaflet
map of the result and serves it (see main_lines_viewer.py).
...
def parse_args(argv=None):
    ...
    parser.add_argument("--viewer", action="store_true",
                        help="Build a Leaflet map of the result and serve it.")
    parser.add_argument("--viewer-port", type=int, default=None,
                        help="Port for --viewer. Default: the viewer's own.")
...
def coded_value_domains(metadata):
    """{field name: {code as text: label}} for the layer's coded-value domains.

    The codes a query returns are integers with no meaning on their own - a
    `material` of 9 says nothing. The layer's own metadata carries the lookup,
    so the label is read from there rather than assumed from another service's
    domain.
    """
    domains = {}
    for field in metadata.get("fields") or []:
        ...
        if not name or str(domain.get("type") or "").lower() != "codedvalue":
            continue
        values = {
            str(entry.get("code")): entry.get("name")
...
# Suffix for the decoded label. A new column rather than an overwrite, so the
# raw code stays in the export for anyone joining on it.
LABEL_SUFFIX = "_LABEL"


def normalize_attributes(attributes, date_fields, domains=None):
    """Convert epoch dates to ISO text and add a label column per coded field."""
    lowered = {str(name).lower() for name in date_fields}
    domains = domains or {}
    normalized = {}
    for key, value in (attributes or {}).items():
        normalized[key] = epoch_ms_to_iso(value) if str(key).lower() in lowered else value
        lookup = domains.get(key)
        if lookup is not None:
            normalized[key + LABEL_SUFFIX] = lookup.get(str(value), "")
    return normalized
...
def write_csv(features, path, date_fields, domains=None):
    rows = [normalize_attributes(f.get("attributes"), date_fields, domains)
            for f in features]
    columns = column_order([{"attributes": row} for row in rows])
...
def write_geojson(features, path, date_fields, domains=None):
    written = []
    skipped = 0
    for feature in features:
        ...
        written.append({
            "type": "Feature",
            "geometry": geometry,
            "properties": normalize_attributes(
                feature.get("attributes"), date_fields, domains),
        })
...
def main(argv=None):
    ...
    if domains:
        log(f"Decoding {len(domains)} coded field(s): {sorted(domains)}")

    write_csv(features, out_dir / f"{args.basename}.csv", dates, domains)
    geojson_path = out_dir / f"{args.basename}.geojson"
    if not args.no_geojson:
        write_geojson(features, geojson_path, dates, domains)

    log(f"Done. {len(features):,} features exported.")

    if args.viewer:
        if args.no_geojson:
            warn("--viewer needs the GeoJSON, so it cannot be used with --no-geojson.")
            return 1
        show_viewer(geojson_path, out_dir, args.viewer_port)
    return 0


def show_viewer(geojson_path, out_dir, port=None):
    """Build the Leaflet map of this export and serve it.

    Imported here rather than at module scope so the export does not depend on
    the viewer being importable.
    """
    import main_lines_viewer

    viewer_dir = Path(out_dir) / f"{Path(geojson_path).stem}_viewer"
    main_lines_viewer.build(geojson_path, viewer_dir)
    main_lines_viewer.serve(viewer_dir, port or main_lines_viewer.DEFAULT_PORT)
